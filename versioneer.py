...
def render_pep440_mtjcustom(pieces):
    branch = pieces.get("branch", "unknown") + "."
    if pieces["closest-tag"]:
        rendered = pieces["closest-tag"]
        if pieces["distance"] or pieces["dirty"]:
            rendered += plus_or_dot(pieces) + branch
            rendered += "%d.g%s" % (pieces["distance"], pieces["short"])
            if pieces["dirty"]:
                rendered += ".dirty"
    else:
        # exception #1
        rendered = "0+untagged." + branch
        rendered += "%d.g%s" % (pieces["distance"], pieces["short"])
        if pieces["dirty"]:
            rendered += ".dirty"
    return rendered
...
def render(pieces, style):
    ...
    elif style == "pep440-mtjcustom":
        rendered = render_pep440_mtjcustom(pieces)
